...
enum SampleData {
    static let manifest = ManifestFile(
        ...
        response: ManifestFileResponse(
            ...
            body: ManifestFileResponse.ResponseBody(json: [
                AnyCodable("data"),
                AnyCodable("redditorInfoByName"),
                AnyCodable("karma"),
                AnyCodable("total")
            ])
        )
    ...
    static let manifestUrl = URL(string: "https://raw.githubusercontent.com/pluto/attest-integrations/refs/heads/main/integrations/reddit-user-karma/manifest.json")!
}
